...
Public worker: registers with BeamCore (bittensor wallet) and runs transfers.
Simple/hidden worker (WORKER_HIDDEN=true): transfer + cache only — no bittensor.

Prefer the slim entrypoint for simple-workers (never imports bittensor):
    python3 simple_worker.py --env-file config/workers/hidden1.env

Public worker:
    pip install 'bittensor>=10.3.1,<11.0.0' httpx websockets
    python3 worker.py --env-file config/workers/worker1.env

Simple worker deps only:
...
# Bittensor is loaded lazily only for public workers (see _import_bittensor).
# Simple/hidden workers and BEAM_SKIP_WORKER_BOOTSTRAP transfer imports never load it.
bt: Any = None
BITTENSOR_AVAILABLE: Optional[bool] = None


def _import_bittensor() -> Any:
    """Import bittensor on demand. Not used by simple/hidden workers."""
    global bt, BITTENSOR_AVAILABLE
    if BITTENSOR_AVAILABLE is True and bt is not None:
        return bt
    if BITTENSOR_AVAILABLE is False:
        raise RuntimeError(
            "bittensor library not installed. Install with: pip install bittensor"
        )
    try:
...
# Hidden/simple worker: transfer + cache only (no BeamCore wallet/register/task_result).
...
async def handle_ws_transfer_offer(state: WorkerState, websocket, task: dict) -> bool:
    """Hidden worker: execute transfer only (orchestrator owns BeamCore task_result)."""
        ...
        try:
            ...
            if status == 403:
                print(
                    ...
                    "    or embedded_global (or global-gateway) that serves /ws/{worker_id}\n"
                    "  - WORKER_GATEWAY_SECRET matches the gateway's WORKER_GATEWAY_SECRET\n"
                    "  - Orchestrator WORKER_GATEWAY_MODE=embedded rejects external WS "
                    "(use embedded_global for hidden workers)\n"
...
def _add_wallet_subtensor_args(parser: argparse.ArgumentParser) -> None:
    ...
    """
    _bt = _import_bittensor()
    wallet_cls = getattr(_bt, "Wallet", None)
    subtensor_cls = getattr(_bt, "Subtensor", None)
...
def get_config():
    ...
    config_cls = getattr(_bt, "Config", None)
...
def load_worker_wallet(config: Any):
    ...
    try:
        ...
        pass
    try:
        return _bt.Wallet(config=config)
    except Exception as cfg_exc:
        raise RuntimeError(
            "Failed to load bittensor Wallet "
            f"(bittensor={getattr(_bt, '__version__', '?')}, "
...
async def main():
    ...
    wallet = None
    if WORKER_HIDDEN:
        print("Simple/hidden worker: transfer+cache only (bittensor not loaded)")
        ...
        try:
            ...
            print(f"Error: {exc}")
            sys.exit(1)
        print(f"bittensor={getattr(_bt, '__version__', '?')}")
        try:
        ...
        if network not in ("finney", "mainnet"):
            ...
            if not api_url:
                raise RuntimeError(
                    "CORE_SERVER_URL is required when running the public worker "
                    "on a non-mainnet network"
                )
    ...
    if api_url:
        ...
        print("API URL: (unused in simple-worker mode)")
    else:
