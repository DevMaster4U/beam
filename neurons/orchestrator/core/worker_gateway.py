...
class WorkerGateway:
    ...
    def connect(self, worker_id: str, ws: object, *, ip: str = "", hidden: bool = False) -> bool:
        ...
        logger.info(
            "Worker connected: %s hidden=%s version=%s ip=%s (%d/%d) queue_cleared=true",
            worker_id,
    ...
    def select_hidden_worker_round_robin(
        self,
    ...
    async def deliver_transfer_offer(self, worker_id: str, offer: dict) -> bool:
        ws = self._sessions.get(worker_id)
        if ws is None:
            logger.warning("deliver_transfer_offer: worker %s not connected", worker_id)
            return False
        try:
        ...
        def _eligible(
            ...
            *,
            ...
            if worker_id in excluded:
                return False
            if self._get_profile(worker_id).hidden:
                return False
    ...
    async def handle_worker_message(self, worker_id: str, raw: str) -> None:
        ...
        if msg_type == "worker_hello":
            ...
            log_relay(
                f"worker ws <- recv type=transfer_result worker={short_id(worker_id)} "
                f"task={short_id(msg.get('task_id'))} "
                f"offer={short_id(msg.get('offer_id') or msg.get('task_id'))} "
            ...
            if handler is not None:
                asyncio.create_task(
                    ...
                    name=f"transfer-result-{worker_id[:8]}",
                )
        else:
