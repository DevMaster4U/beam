...
class SubnetCoreClient:
    def __init__(
        ...
        self._embedded_worker_pool = None
        self._global_gateway_client = None
    ...
    def _registration_ready(self) -> bool:
        ...
        if self._embedded_worker_pool is not None:
            if int(getattr(self._embedded_worker_pool, "worker_count", 0) or 0) > 0:
                return True
        if self._global_gateway_client is not None:
            # Shared pool modes: operator ready is enough; pool size is synced separately.
            return True
    ...
    async def _handle_task_offer_batch(self, data: dict) -> None:
        ...
        if not self._has_delivery_target():
            logger.warning("No worker delivery target available for batch %s", batch_id)
            return
        if self._task_offer_dispatcher:
    ...
    async def _deliver_task_offer_batch_to_workers(self, data: dict) -> None:
        ...
        for offer in valid_offers:
            ...
            if await self._worker_gateway.deliver_task_offer(worker_id, offer):
                ...
                logger.warning(
                    "Failed to forward task offer: batch=%s worker=%s task=%s",
                    batch_id,
                    worker_id,
                    offer.get("task_id"),
                )
    ...
    _task_accept_removed_logged = False
    ...
    async def send_task_accept(
        ...
        """No-op: BeamCore protocol is result-only (task_accept removed)."""
        del worker_version  # unused; kept for call-site compatibility
        resolved_offer = offer_id or task_id
        if not SubnetCoreClient._task_accept_removed_logged:
            SubnetCoreClient._task_accept_removed_logged = True
            logger.info(
                "send_task_accept is a no-op (BeamCore task_accept removed; result-only protocol)"
            )
        return {
    ...
    async def get_worker_hotkey(self, worker_id: str) -> Optional[str]:
        """Resolve worker hotkey via BeamCore HTTP (NATS has no get_worker_hotkey)."""
        try:
            client = await self._get_client()
            resp = await client.get(f"{self.base_url}/workers/{worker_id}/hotkey")
            if resp.status_code != 200:
                return None
            data = resp.json()
            return data.get("hotkey") or data.get("worker_hotkey")
        except Exception as exc:
            logger.warning("get_worker_hotkey failed for %s: %s", worker_id, exc)
            return None

    async def clear_history(self) -> dict:
        """Clear orchestrator transfer history via BeamCore HTTP."""
        try:
            client = await self._get_client()
            resp = await client.delete(f"{self.base_url}/orchestrators/{self.orchestrator_hotkey}/history")
